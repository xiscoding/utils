...
DATA_CACHE_DIR = "data"
DATA_SET_NAME = "alpaca_cota"
DATA_SET_URL = "https://huggingface.co/datasets/QingyiSi/Alpaca-CoT/resolve/main/Auto-CoT/Auto-CoT.json"
DATA_SET_FILE_NAME = "Auto-CoT.json"
...
def download():
    ...
    data_filename = os.path.join(data_dir, DATA_SET_FILE_NAME)
...
def pretokenize():
    ...
    def process_shard(shard):
        ...
        for example in tqdm(data):
            ...
            if example["input"].strip():  # checks if "input" contains non-whitespace text
                text += " " + example["input"]  # concatenate instruction and input
            text = text.strip()  # get rid of leading/trailing whitespace
            tokens = enc.encode(text, bos=True, eos=False)  # encode the text, use BOS
            all_tokens.extend(tokens)
        # convert to uint16 nparray
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        list(executor.map(process_shard, shard_filenames))  # Collect results to ensure exceptions are raised
...
class PretokDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        #NOTE: auto_cot.py path: /home/xdoestech/llama2.c/data/alpaca_cota/Auto-CoT.bin
        data_dir = os.path.join(DATA_CACHE_DIR, DATA_SET_NAME)
